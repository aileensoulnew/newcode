...
class Userprofile_page extends MY_Controller {
    ...
    public function contacts_data() {
        ...
        if(empty($_GET["user_slug"])){
            $userid = $this->session->userdata('aileenuser');
        } else {
            $user_slug = $_GET["user_slug"];
            $userid = $this->user_model->getUserDataByslug($user_slug, $select_data = "u.user_id");
        }
        $contactsData = $this->data['contactsData'] = $this->userprofile_model->getContactData($userid['user_id'], $data = "", $page);
    ...
    public function user_image_insert1() {
        ...
        if ($update) {
            ...
            redirect('profiles/' . $userdata['user_slug'], refresh);
        }
    }
