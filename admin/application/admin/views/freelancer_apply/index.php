 ...
 <!-- Content Header (Page header) -->
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Freelancer Apply User</li>
        </ol>
        <!-- <div class="fr">
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                    ...
                    <div class="box-tools">
                       <?php echo form_open('freelancer_apply/search', array('method' => 'post', 'id' => 'search_frm', 'class' => 'form-inline','autocomplete' => 'off')); ?>
                ...
                <?php echo form_close(); ?>
                    ...
                    ?>
                            ...
                            <a href="<?php echo base_url('freelancer_apply/clear_search') ?>">Clear Search</a>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover table-bordered">
                ...
                ?>
                    ...
                    <a href="<?php echo ( $this->uri->segment(3) == 'freelancer_post_fullname' && $this->uri->segment(4) == 'ASC') ? site_url($this->uri->segment(1) . '/' . $segment2 . '/freelancer_post_fullname/DESC/' . $offset) : site_url($this->uri->segment(1) . '/' . $segment2 . '/freelancer_post_fullname/ASC/' . $offset); ?>"> 
                     Name
                     </a>

                     <?php echo ( $this->uri->segment(3) == 'freelancer_post_fullname' && $this->uri->segment(4) == 'ASC' ) ? '<i class="glyphicon glyphicon-arrow-up">' : (( $this->uri->segment(3) == 'freelancer_post_fullname' && $this->uri->segment(4) == 'DESC' ) ? '<i class="glyphicon glyphicon-arrow-down">' : '' ); ?>
...
<!--                    <th><i class="fa fa-fw fa-venus"></i> 
                ...
                <tr id="delete<?php echo $user['freelancer_post_reg_id']?>">
                    <td><?php echo $i++; ?></td>

                    <td><?php echo ucfirst($user['freelancer_post_fullname']); echo ' ';echo ucfirst($user['freelancer_post_username']);  ?></td>

                    <td><?php echo $user['freelancer_post_email']; ?></td>

                    <td><?php if($user['freelancer_post_phoneno'])
                              {
                                echo $user['freelancer_post_phoneno']; 
                              }
                              else
                              {
                                echo PROFILENA;
                    ...
                    <td> 
                        <?php 
                            ...
                            $cityname = $this->db->get_where('cities', array('city_id' => $user['freelancer_post_city']))->row()->city_name;

                            echo $cityname; if( $cityname){echo ",<br>";}

                            $statename = $this->db->get_where('states', array('state_id' => $user['freelancer_post_state']))->row()->state_name;

                            echo $statename;if( $statename){echo ",<br>";}

                            $countryname = $this->db->get_where('countries', array('country_id' => $user['freelancer_post_country']))->row()->country_name; 
                    ...
                    <td> 
                        <?php  if($user['freelancer_post_user_image']) 
                                {
                        ?>
                                <img src="<?php echo SITEURL . $this->config->item('free_post_profile_thumb_upload_path') . $user['freelancer_post_user_image']; ?>" alt=""  style="height: 70px; width: 70px;">
                        <?php }else{
                        ?>
                    ...
                    <td id="active<?php echo $user['freelancer_post_reg_id']?>">
                        <?php if ($user['status'] == 1) 
                              {
                        ?>
                                    <button class="btn btn-block btn-primary btn-sm"  onclick="deactive_user(<?php echo $user['freelancer_post_reg_id']; ?>);">Active</button>
                        <?php 
                            }else{ ?>

                                        <button class="btn btn-block btn-success btn-sm" onclick="active_user(<?php echo $user['freelancer_post_reg_id']; ?>);">Deactive</button>
                    ...
                    <td>
                        ...
                        <button class="btn btn-danger btn-xs" onclick="delete_user(<?php echo $user['freelancer_post_reg_id']; ?>);">
                        <i class="fa fa-trash-o"></i>
                        </button>

                        <a class="btn btn-success btn-xs" href="<?php echo base_url('freelancer_apply/profile/'.$user['freelancer_post_reg_id'] ); ?>">
...
<script>
//deactive user Start
   function deactive_user(freelancer_post_reg_id) 
   {
       ...
       $.fancybox.open('<div class="message"><h2>Are you Sure you want to  deactive this User?</h2><button id="activate" class="mesg_link btn btn1">OK</a><button data-fancybox-close="" class="btn btn1">Cancel</button></div>');
        ...
        $('.message #activate').on('click', function () 
        {
            $.ajax({
                         type: 'POST',
                          url: '<?php echo base_url() . "freelancer_apply/deactive_user" ?>',
                          data: 'freelancer_post_reg_id=' + freelancer_post_reg_id,
                          success: function (response) 
                          {    
                                 $.fancybox.close();
                                $('#' + 'active' + freelancer_post_reg_id).html(response);
                          }
            });   
        });
    }
...
//active user Start
   function active_user(freelancer_post_reg_id) 
   {
       ...
       $.fancybox.open('<div class="message"><h2>Are you Sure you want to  active this User?</h2><button id="deactivate" class="mesg_link btn btn1">OK</a><button data-fancybox-close="" class="btn btn1">Cancel</button></div>');
        ...
        $('.message #deactivate').on('click', function () 
        {
            $.ajax({
                         type: 'POST',
                          url: '<?php echo base_url() . "freelancer_apply/active_user" ?>',
                          data: 'freelancer_post_reg_id=' + freelancer_post_reg_id,
                          success: function (response) 
                          {        
                                  $.fancybox.close();  
...
//Delete user Start
   function delete_user(freelancer_post_reg_id) 
   {
       ...
       $.fancybox.open('<div class="message"><h2>Are you Sure you want to Delete this User?</h2><button id="delete" class="mesg_link btn btn1">OK</a><button data-fancybox-close="" class="btn btn1">Cancel</button></div>');
        ...
        $('.message #delete').on('click', function () 
        {
            $.ajax({
                         type: 'POST',
                          url: '<?php echo base_url() . "freelancer_apply/delete_user" ?>',
                          data: 'freelancer_post_reg_id=' + freelancer_post_reg_id,
                          success: function (response) 
                          {          
